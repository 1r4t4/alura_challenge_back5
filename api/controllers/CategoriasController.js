...
class CategoriasController {
    ...
    static async pegaUmaCategoria(req, res) {
      ...
      static async pegaTodosVideosDaCategoria(req, res) {
        const { id } = req.params
        try {
            const todosVideosDaCategoria = await database.Videos.findAll(
                {
                    where: {
                        categorias: id
                    }
            ...
            return res.status(200).json(todosVideosDaCategoria)
        } catch (error) {
            return res.status(500).json(error.message)
            }
        }

      static async criaNovaCategoria(req, res) {
